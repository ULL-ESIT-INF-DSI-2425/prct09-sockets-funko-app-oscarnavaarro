...
function sendRequest(request: requestType) {
  ...
  client.on("end", () => {
    ...
    try {
      ...
      if (response.success) {
        console.log(
          chalk.green("Success: ") +
            (response.message || `Operation ${response.type} completed.`),
        );
        ...
        console.error(
          chalk.red("Error: ") +
            (response.message || `Operation ${response.type} failed.`),
        );
      }
    } catch (error) {
  ...
  client.on("error", (err) => {
    ...
    if ((err as any).code === "ECONNREFUSED") {
      console.error(
        chalk.red(
          `Could not connect to server at ${SERVER_HOST}:${SERVER_PORT}. Is the server running?`,
        ),
      );
    }
  });
...
yargs(hideBin(process.argv))
  .command({
    ...
    builder: {
      user: {
        type: "string",
        demandOption: true,
        describe: "Username of the collection owner",
      },
      id: {
        type: "number",
        demandOption: true,
        describe: "Unique ID for the Funko",
      },
      name: {
        type: "string",
        demandOption: true,
      ...
      franchise: {
        type: "string",
        demandOption: true,
        describe: "Franchise the Funko belongs to",
      },
      number: {
        type: "number",
        demandOption: true,
        describe: "Number within the franchise",
      },
      exclusive: {
        type: "boolean",
        demandOption: true,
      ...
      specialFeatures: {
        ...
        demandOption: true,
        default: "None",
        describe: "Special features (e.g., Glows in the dark)",
      },
      marketValue: {
        type: "number",
        demandOption: true,
  ...
  .command({
    ...
    builder: {
      ...
      id: {
        type: "number",
        demandOption: true,
        describe: "ID of the Funko to update",
      },
    ...
    handler: (argv) => {
      ...
      if (argv.type !== undefined)
        potentialUpdate.type = argv.type as FunkoType;
      if (argv.genre !== undefined)
        potentialUpdate.genre = argv.genre as FunkoGenre;
      if (argv.franchise !== undefined)
        potentialUpdate.franchise = argv.franchise;
      if (argv.number !== undefined) potentialUpdate.number = argv.number;
      if (argv.exclusive !== undefined)
        potentialUpdate.exclusive = argv.exclusive;
      if (argv.specialFeatures !== undefined)
        potentialUpdate.specialFeatures = argv.specialFeatures;
      if (argv.marketValue !== undefined)
        potentialUpdate.marketValue = argv.marketValue;
      ...
      if (Object.keys(potentialUpdate).length <= 1) {
        console.error(
          chalk.red(
            "Error: You must provide at least one property to update (e.g., --name, --marketValue).",
          ),
        );
      ...
      console.log(
        chalk.yellow(
          "Note: Sending partial update data. Server will merge with existing Funko.",
        ),
      );
      sendRequest(request);
    },
  ...
  .command({
    ...
    builder: {
      ...
      id: {
        type: "number",
        demandOption: true,
        describe: "ID of the Funko to remove",
      },
    },
    handler: (argv) => {
      const request: requestType = {
  ...
  .command({
    ...
    builder: {
      ...
      id: {
        type: "number",
        demandOption: true,
        describe: "ID of the Funko to read",
      },
    },
    handler: (argv) => {
      const request: requestType = {
  ...
  .demandCommand(
    1,
    chalk.red(
      "You need to provide a command (add, update, remove, list, read).",
    ),
  )
  .help()
  .alias("h", "help")
